...
public partial class QuizRunner : IDisposable {
    ...
    private PrepareExamSessionRequestDto ExamSessionRequest { get; set; } = new PrepareExamSessionRequestDto();
    ...
    protected async Task PrepareExamSessionSubmittedHandler(PrepareExamSessionRequestDto input) {
        ...
        if (!input.ExamId.HasValue) {
            ...
            _ = _uiAppService.ShowAlert("Please, select an exam");
            return;
        }
        ...
        try {
            ...
            if (output.TotalCount <= 0) {
                ...
                _ = _uiAppService.ShowAlert("No available question for the selected exam and options");
                return;
            }
    ...
    private async Task RestartExamSessionClick(MouseEventArgs evt) {
        if ((ExamSession == null) || (ExamSession.TotalAnswers <= 0)) {
            ...
            _ = _uiAppService.ShowAlert("Oops! There is no user session to restart");
            return;
        }
    ...
    private void HandleReset() {
        try {
            if (ExamSession != null) ExamSession.OnChange -= StateHasChanged;
            ...
            ExamSession.OnChange += StateHasChanged;
        }
        finally {
        }
    }
    ...
    private async Task EndAndSubmitExamSession(MouseEventArgs evt) {
        if ((ExamSession == null) || (ExamSession.TotalAnswers <= 0)) {
            ...
            _ = _uiAppService.ShowAlert("Oops! There is no user session to save");
            return;
            ...
            _ = _uiAppService.ShowAlert("Oops! Exam session has already been submitted");
            return;
        }
        ...
        try {
            ...
            _ = _uiAppService.ShowAlert("Oops! An error occurred trying to execute the requested operation: " + ex.Message);
        }
        finally {
            _uiAppService.HideActivityIndicator();
        }
